# Networking Module for Zama API Platform Challenge
...
resource "aws_security_group" "alb" {
  ...
  ingress {
    description = "Kong Konnect Cloud"
    from_port   = 8080
    to_port     = 8080
    protocol    = "tcp"
    cidr_blocks = ["205.234.240.62/32"] # Kong's current IP
  }

  # Your management IP for testing
  ingress {
    description = "Management access"
    from_port   = 8080
    to_port     = 8080
    protocol    = "tcp"
    cidr_blocks = ["88.181.233.30/32"] # Your current IP
  }

  # Health checks from ALB (internal VPC only)
  ingress {
...
resource "aws_security_group" "ecs_tasks" {
  ...
  # Kong Gateway port from ALB
  ingress {
    description     = "Kong Gateway from ALB"
    from_port       = 8000
    to_port         = 8000
    protocol        = "tcp"
    security_groups = [aws_security_group.alb.id]
